...
public class DisplayRoster implements IDisplayRoaster {
    ...
    public void displayFreeAgentsToBeHired(ArrayList<IPlayer> freeAgents, int count) {
        ...
        for (int i = 0; i < freeAgents.size(); i++) {
            System.out.format("%-5d %-20s %-20s %8d %20d %20d %20d",
                    i + 1, freeAgents.get(i).getPlayerName(), freeAgents.get(i).getPlayerPosition(), freeAgents.get(i).getPlayerSkating(), freeAgents.get(i).getPlayerShooting(), freeAgents.get(i).getPlayerChecking(), freeAgents.get(i).getPlayerSaving());
            System.out.println();
        }
